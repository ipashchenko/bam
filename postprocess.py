...
def postprocess(temperature=1., numResampleLogX=1, plot=True, loaded=(), cut=0.,
    ...
    logl_samples = [(sample_info[i, 1], sample_info[i, 2], i) for i in
                    range(0, sample_info.shape[0])]  # logl, tiebreaker, id
    ...
    for z in range(0, numResampleLogX):
        ...
        if plot:
            ...
            plt.ylabel('log(L)')
            plt.title(str(z+1)+"/"+str(numResampleLogX)+", log(Z) = "+str(
                logz_estimates[z][0]))
    ...
    for i in range(len(P_samples)):
        logp_samples_averaged = logsumexp(logp_samples[i, :]) - np.log(logp_samples.shape[1])
...
def levels_plot():
    ...
    plt.xlabel('Level')
    plt.ylabel('$\lg$(Delta log likelihood)')
    plt.show()
